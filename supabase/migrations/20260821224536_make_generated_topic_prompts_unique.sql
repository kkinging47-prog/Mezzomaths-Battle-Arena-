  ...
  loop
    for i in 1..500 loop
      ...
      if kind='addition' then
        a := case when cfg.class_level='Grade 1' then (i-1)%50+1 when d=1 then 20+i else 100*d+i*3 end;
        b := case when cfg.class_level='Grade 1' then floor((i-1)/10)+1 when d=1 then 5+floor(i/3) else 40*d+i*2 end;
        ans := a+b;
        q := case mode
        ...
        b := case when d=1 then 3+((i-1)%40) when d=2 then 30+i else 100+i*2 end;
        a := b + case when d=1 then 5+floor((i-1)/5) when d=2 then 80+i*2 else 300+i*5 end;
        ans := a-b;
        q := case mode
          when 1 then format('Calculate %s − %s.',a,b)
        ...
        if i%2=0 then
          a := 20*d+i*3; b := 5*d+i; c := 2*d+((i-1)%20);
          ans := a+b-c;
          q := case mode
          ...
          b := 10*d+i*2; c := 3*d+((i-1)%15); a := b+c+20*d+i;
          ans := a-b+c;
          q := case mode
            when 1 then format('Evaluate %s − %s + %s.',a,b,c)
        ...
        b := case kind when 'multiply10' then 10 when 'multiply11' then 11 else case when d=1 then 2+((i-1)%9) when d=2 then 11+((i-1)%10) else 21+((i-1)%20) end end;
        a := case when d=1 then 2+i when d=2 then 20+i*2 else 100+i*3 end;
        ans := a*b;
        q := case mode
          when 1 then format('Calculate %s × %s.',a,b)
        ...
        ans := case when d=1 then 2+i when d=2 then 50+i*2 else 200+i*3 end;
        a := ans*b;
        q := case mode
          when 1 then format('Calculate %s ÷ %s.',a,b)
        ...
        a := 360 + ((i*7 + d*13) % 540);
        b := case when d=1 then 15*((i-1)%4+1) when d=2 then 25+5*((i-1)%18) else 65+5*((i-1)%30) end;
        ans := a+b;
        ...
        a := 1 + ((i-1) % 9);
        b := a + 1 + (floor((i-1)/9)::int % 9);
        c := 1 + (floor((i-1)/81)::int % 7);
        ans := c + 1 + (floor((i-1)/567)::int % 5);
        if a::numeric/b > c::numeric/ans then ans_text := '>'; elsif a::numeric/b < c::numeric/ans then ans_text := '<'; else ans_text := '='; end if;
        q := format('Choose the correct sign: %s/%s __ %s/%s.',a,b,c,ans);
        expl := format('Using a common denominator shows that %s/%s %s %s/%s.',a,b,ans_text,c,ans);

      elsif kind='geometry' then
        if mode in (1,2) then
          a := 3*d+i; b := 2*d+((i-1)%40)+1; ans := a*b;
        ...
        a := 30*d+i*4; b := 5*d+i; c := 2*d+((i-1)%25);
