...
function exportTeacherReport() { const h = historyRows(); const rows = [['Student','School','Class','Location','Mode','Topic','Score','Total','Date'], ...h.map(x => [x.student,x.school,x.classLevel,x.location,x.mode,x.topic,x.score,x.total,x.date])]; const csv = rows.map(row => row.map(cell => `"${String(cell || '').replaceAll('"','""')}"`).join(',')).join('\n'); const blob = new Blob([csv], { type: 'text/csv' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'mezzo-teacher-dashboard-report.csv'; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000) }
function syncAll() { if (syncQueued) return; syncQueued = true; requestAnimationFrame(() => { syncQueued = false; enhanceAdminForms(); syncAuthRoles(); syncNavRoleButton(); hideNonSmartboardAutoMatch() }) }
